...
template<typename VALUE_TYPE>
std::pair<VALUE_TYPE, std::vector<size_t>>
greedy_joining_spanning_forest(size_t n, std::vector<std::array<size_t, 2>> edges, std::vector<VALUE_TYPE> edge_values, double tau, char mode){
    ...
    std::cout << "Frac: " << frac << "\n";
    std::chrono::duration<double> total_graph(0);  
    ...
    total_graph += std::chrono::duration<double>(end_graph - start_graph);
      
    std::chrono::duration<double> total_init(0);
    std::chrono::duration<double> total_sort(0);
    std::chrono::duration<double> total_kruskal(0);
    std::chrono::duration<double> total_update(0);
    ...
    while(1){
        ...
        total_init += std::chrono::duration<double>(end_init - start_init);
    ...
    print_edges(positive_edges);*/
        ...
        if(positive_edges.empty()){
            std::vector<size_t> labeling(n);
            partition.elementLabeling(labeling.begin());
            std::cout << "Gesamtzeit Initialisierung des Graphen: " << total_graph.count() << "s\n";
            std::cout << "Gesamtzeit Extrahierung der Kanten: " << total_init.count() << "s\n";
            std::cout << "Gesamtzeit Sortierung: " << total_sort.count()<< "s\n";
            std::cout << "Gesamtzeit Kruskal: " << total_kruskal.count()<< "s\n";
            std::cout << "Gesamtzeit Graphaktualisierungen: " << total_update.count()<< "s\n";
            return {value_of_cut, labeling};
        }
        ...
        total_sort += std::chrono::duration<double>(end_sort - start_sort);
        ...
        total_kruskal += std::chrono::duration<double>(end_kruskal - start_kruskal);
        /*if(frac < mst.size()){
            mst.resize(frac);
        }*/
        ...
        for (auto [u,v] : mst) {
          ...
          total_update += std::chrono::duration<double>(end_update - start_update);
    ...
    while(1){
        ...
        for (size_t u = 0; u < n; ++u) {
            ...
            if (best_neighbor[v].has_value() && *best_neighbor[v] == u && u < v) {
                ...
                counter++;
                if(counter >= frac) return matches;
